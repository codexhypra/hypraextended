function CreateExtendedPlayer(playerId , identifier , group , accounts , inventory , weight , job , loadout , name , coords)
	...
	self.addInventoryItem      = function(name , count)
		...
		if item then
			...
			TriggerClientEvent('esx_inventoryhud:notification' , self.source , item.name , item.label , count , false)
		end
	end
	...
	self.removeInventoryItem   = function(name , count)
		...
		if item then
			...
			if newCount >= 0 then
				...
				TriggerClientEvent('esx_inventoryhud:notification' , self.source , item.name , item.label , count , true)
			end
		end
	end
	...
	self.setJob                = function(job , grade)
		...
		if ESX.DoesJobExist(job , grade) then
			...
			print(('[HYPRA] [^3WARNING^7] Ignoring invalid .setJob() usage for "%s"'):format(self.identifier))
		end
	end
