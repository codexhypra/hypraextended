...
ESX.RegisterCommand         = function(name , group , cb , allowConsole , suggestion)
	...
	if ESX.RegisteredCommands[name] then
		print(('[HYPRA] [^3WARNING^7] An command "%s" is already registered, overriding command'):format(name))
	...
	RegisterCommand(name , function(playerId , args , rawCommand)
		...
		if not command.allowConsole and playerId == 0 then
			print(('[HYPRA] [^3WARNING^7] %s'):format(_U('commanderror_console')))
		else
			...
			if error then
				if playerId == 0 then
					print(('[HYPRA] [^3WARNING^7] %s^7'):format(error))
				else
					xPlayer.triggerEvent('chat:addMessage' , { args = { '^1[HYPRA]', error } })
				end
			else
				cb(xPlayer or false , args , function(msg)
					if playerId == 0 then
						print(('[HYPRA] [^3WARNING^7] %s^7'):format(msg))
					else
						xPlayer.triggerEvent('chat:addMessage' , { args = { '^1[HYPRA]', msg } })
					end
				end)
			end
		end
...
ESX.TriggerServerCallback   = function(name , requestId , source , cb , ...)
	if ESX.ServerCallbacks[name] then
		...
		print(('[HYPRA] [^3WARNING^7] Server callback "%s" does not exist. Make sure that the server sided file really is loading, an error in that file might cause it to not load.'):format(name))
	end
end
...
ESX.SavePlayer              = function(xPlayer , cb)
	...
	table.insert(asyncTasks , function(cb2)
		MySQL.Async.execute('UPDATE users SET name = @nombre, accounts = @accounts, job = @job, job_grade = @job_grade, `group` = @group, loadout = @loadout, position = @position, inventory = @inventory WHERE identifier = @identifier' , {
			['@nombre']     = playerName,
	...
	Async.parallel(asyncTasks , function(results)
		print(('[^2INFO^7] Jugador Guardado "%s^7"'):format(playerName))
...
ESX.SavePlayers             = function(cb)
	...
	Async.parallelLimit(asyncTasks , 8 , function(results)
		print(('[HYPRA] [^2INFO^7] Saved %s player(s)'):format(#xPlayers))
		if cb then
			cb()
		end
	end)
...
--[[Framework.Functions.IsPlayerBanned = function(source)
	...
	Framework.Functions.ExecuteSql(true, "SELECT * FROM `white_playersinfo` WHERE `steam` = '"..GetPlayerIdentifiers(source)[1].."' OR `license` = '"..GetPlayerIdentifiers(source)[2].."' OR `license` = '"..GetPlayerIdentifiers(source)[3].."'", function(result)
		if result[1] ~= nil then
			Message = "\n🔰 Ha sido expulsado del servidor.\n🛑 Razón: " ..result[1].reason.. '\n🛑 Prohibido por: ' ..result[1].bannedby.. '\n\n Para desbancar, puede abrir un ticket en la discordia.'
			IsBanned = true
		else
			IsBanned = false
		end
