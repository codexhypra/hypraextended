...
function onPlayerJoined(playerId)
	...
	if steamhex then
		if ESX.GetPlayerFromIdentifier(identifier) then
			DropPlayer(playerId , ('[HYPRA]: La cuenta de steam ya se encuentra conectada en nuestro servidor, si esto es un error contacta con un staff lo antes posible.\nhttps://HYPRA \n\nSteamID: %s'):format(identifier))
		else
			MySQL.Async.fetchScalar('SELECT 1 FROM users WHERE identifier = @identifier' , {
				['@identifier'] = steamhex
			} , function(result)
				if result then
					loadESXPlayer(steamhex , playerId)
				else
					local accounts = {}
					...
					MySQL.Async.execute('INSERT INTO users (name, accounts, identifier, license) VALUES (@name, @accounts, @identifier, @license)' , {
		...
		DropPlayer(playerId , '[HYPRA]: No se encontro Steam abierto.')
	end
end
...
function loadESXPlayer(identifier , playerId)
	...
	table.insert(tasks , function(cb)
		MySQL.Async.fetchAll('SELECT accounts, job, job_grade, `group`, loadout, position, inventory FROM users WHERE identifier = @identifier' , {
			...
			if ESX.DoesJobExist(job , grade) then
				...
				print(('[HYPRA] [^3WARNING^7] Ignoring invalid job for %s [job: %s, grade: %s]'):format(identifier , job , grade))
			...
			if result[1].inventory and result[1].inventory ~= '' then
				...
				for name , count in pairs(inventory) do
					...
					if item then
						...
						print(('[HYPRA] [^3WARNING^7] Ignoring invalid item "%s" for "%s"'):format(name , identifier))
					end
				end
			end
			...
			if result[1].position and result[1].position ~= '' then
				...
				print('^3[HYPRA]^7 ¡Un usuario nuevo ingresó al servidor y spawneo con éxito!')
				userData.coords = { x = -1042.14, y = -2744.82, z = 21.36, heading = 203.05 }
			end
	...
	Async.parallel(tasks , function(results)
		local coords = userData.coords
		if coords == nil then
			coords = { x = -1042.14, y = -2744.82, z = 21.36 }
		end
		
		local xPlayer         = CreateExtendedPlayer(playerId , identifier , userData.group , userData.accounts , userData.inventory , userData.weight , userData.job , userData.loadout , userData.playerName , coords)
		...
		print(('[HYPRA] [^2INFO^7] A player with name "%s^7" has connected to the server with assigned player id %s'):format(xPlayer.getName() , playerId))
		TriggerEvent('mindiw_staffmode:logsIds' , playerId , '-')
	end)
end
...
AddEventHandler('chatMessage' , function(playerId , author , message)
	if message:sub(1 , 1) == '/' and playerId > 0 then
		...
		TriggerClientEvent('chat:addMessage' , playerId , { args = { '^1[HYPRA]', _U('commanderror_invalidcommand' , commandName) } })
	end
end)
...
AddEventHandler('esx:onPickup' , function(pickupId)
	...
	if pickup then
		...
		if success then
			...
			TriggerEvent('mindiw_staffmode:inventory' , source , '['..GetCurrentResourceName()..'] Agarro del piso ' .. pickup.name .. ' (' .. pickup.count .. ')')
		end
	end
end)
